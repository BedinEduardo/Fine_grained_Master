...
def split_data(args):  #, val_ratio): #chama o args via main o val_ratio vem junto 	#Criando o inicio da Funcao
...
#os.makedirs('./data/val/', exist_ok=True)
	...
	for fold in range (1, args.num_folds+1):   #the condition to create the train ans test folders - run the numbers of defined folers
	
		print(f"Press any key to continue to fold {fold}")   #condition to continue - in the code it will be replaced by the end of fold training - TIRAR ESSA APRTE AQUI QDO RODAR NO CODIGO COMPLETO
	...
	#aqui vai o val_set_folder = 
		...
		for train_fold in range(1, args.num_folds + 1):  #contador para contar od folds
...
def transfer_to_results(fold, test_folders, train_folders, val_set_folder, test_set_folder, train_set_folder):
