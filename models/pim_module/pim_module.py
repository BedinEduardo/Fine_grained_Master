...
class FPN(nn.Module):
    ...
    def upsample_add(self, x0: torch.Tensor, x1: torch.Tensor, x1_name: str):
        ...
        """
        #verificando os tamanhos de X0 e X1
        print("X0: ", x0.shape)
        print("X1: ", x1.shape)
        
        if self.upsample_type == "Bilinear":
            if x1.size(-1) != x0.size(-1) or x1.size(-2) != x0.size(-2):  #colocando um 'or' q nao entendi porque
                #x1 = self.upsample(x1)  #esse eh o original
                #x1 = self.upsample(x1, size=(x0.size(-2), x0.size(-1)))  # Adjust spatial dimensions #PARACE Q FUNCIONOU - TESTAR ATEH O FINAL
                x1 = F.interpolate(x1, size=(x0.size(-2), x0.size(-1)), mode='bilinear', align_corners=False)
        else:
...
class PluginMoodel(nn.Module):
    ...
    def forward_backbone(self, x):
        return self.backbone(x)  #esta dandp erro nessa linha
